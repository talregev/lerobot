...
from lerobot.envs.configs import GymnasiumEnv

def create_gymnasium_envs(
    cfg: GymnasiumEnv,
    n_envs: int = 1,
    use_async_envs: bool = False,
) -> Dict[str, Dict[int, gym.vector.VectorEnv]]:
    """
    Build vectorized GymnasiumEnv(s) from the GymnasiumEnv config and return:
    ...
    def _mk_one(worker_idx: int):
        def _ctor():
            ...
            return GymnasiumEnv(task=task, seed=seed, image_key=image_key)
        return _ctor
    ...
    return {"gymnasium": {0: vec_env}}

class GymnasiumEnv(gym.Env):
    """Minimal adapter: wraps a Gymnasium env and returns a LeRobot-style obs dict."""
